...
const UserPurchasedProduct = () => {
    const user_id = StorageHelper.getUserData()?.id;

    const { data, isLoading } = useQuery(
        ["wishlist-purchased-gift", user_id],
        () =>
          WishlistServices.UserPurchasedGift({id:
             `${user_id}`}
          ),
        {
          ...
          onSuccess: (data) => {
            ...
            console.log("User Data Purchased Gift",data?.data
                ,"User Data")
      ...
      <div className="row">
        <div className="col-12 mb-4">
          <div className="comon-title">
            <h3><strong>Purchased Gifts</strong></h3>
          </div>
        </div>
      </div>
      <div className="row" id="focusme">
        {isLoading ? (
